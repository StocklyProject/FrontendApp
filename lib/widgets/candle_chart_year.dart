...
class CandleChartYear extends StatefulWidget {
  final String symbol;
  final Map<String, dynamic>? newData; // 상위 위젯에서 받은 실시간 주식 데이터

  const CandleChartYear({
    Key? key,
  ...
  @override
  _CandleChartYearState createState() => _CandleChartYearState();
}

class _CandleChartYearState extends State<CandleChartYear> {
  ...
  Future<void> fetchInitial() async {
    try {
      final url = Uri.parse(
          'http://localhost.stock-service/api/v1/stockDetails/historicalFilter?symbol=${widget.symbol}&interval=1y');
      ...
      if (response.statusCode == 200) {
        ...
        final parsedData = data.map((item) {
          return CandleData(
            timestamp: DateTime.parse('${item['year']}-01-01').millisecondsSinceEpoch,
  ...
  @override
  void didUpdateWidget(covariant CandleChartYear oldWidget) {
