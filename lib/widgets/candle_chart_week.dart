...
class CandleChartWeek extends StatefulWidget {
  final String symbol;
  final Map<String, dynamic>? newData; // 상위 위젯에서 받은 실시간 주식 데이터

  const CandleChartWeek({
    Key? key,
  ...
  @override
  _CandleChartWeekState createState() => _CandleChartWeekState();
}

class _CandleChartWeekState extends State<CandleChartWeek> {
  ...
  Future<void> fetchInitial() async {
    try {
      final url = Uri.parse(
          'http://localhost.stock-service/api/v1/stockDetails/historicalFilter?symbol=${widget.symbol}&interval=1w');
      ...
      if (response.statusCode == 200) {
        ...
        final parsedData = data.map((item) {
          return CandleData(
            timestamp: DateTime.parse(item['end_date']).millisecondsSinceEpoch,
  ...
  @override
  void didUpdateWidget(covariant CandleChartWeek oldWidget) {
